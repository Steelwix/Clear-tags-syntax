            ...
            foreach ($tags as $checkedTag){
                if($tag == $checkedTag){
                    ...
                    foreach ($keys as $key=>$value){
                        if($tagKey != $value){
                            echo '<pre style="background-color:orange">', 'DETECTED DOUBLE TAGS ON ',var_dump($tags[$tagKey]),var_dump($tagKey),var_dump($tags[$value]),var_dump($value), '</pre>';
...
//        fclose($file);
    ...
    public function updateTagsArray(array $array, ?string $oldValue = null, ?string $newValue = null): array{
        ...
        echo '<pre style="background-color:#4dd4ac">', 'tag updates ', var_dump($array), '</pre>';
